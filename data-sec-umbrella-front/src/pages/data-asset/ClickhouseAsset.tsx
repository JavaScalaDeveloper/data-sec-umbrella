...
import { dataSourceApi, clickhouseDatabaseApi, clickhouseTableApi, clickhouseAssetScanApi, databasePolicyApi } from '../../services/api';
...
const ClickhouseAsset: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'instance' | 'database' | 'table'>(() => {
    ...
    if (path.endsWith('/database-security/data-asset/clickhouse/database')) {
      return 'database';
    }
    if (path.endsWith('/database-security/data-asset/clickhouse/table')) {
      return 'table';
    }
    return 'instance';
  });
    ...
    },
    {
      ...
      render: (_text, record) => (
        <Select
          ...
          onChange={async (v) => {
            try {
              const res = await clickhouseTableApi.updateManualReview({
                id: record.id,
                manualReview: v,
              });
              if (res.code === 200) {
  ...
  // 手动触发 ClickHouse 资产扫描
  const handleScan = async () => {
    try {
      message.info('开始扫描 ClickHouse 资产...');
      const response = await clickhouseAssetScanApi.scan();
      if (response.code === 200) {
        message.success('ClickHouse 资产扫描完成');
        // 重新加载数据
        fetchInstances();
        fetchDatabases();
        fetchTables();
  ...
  useEffect(() => {
    const path = location.pathname;
    // 兼容旧入口：/database-security/data-asset/clickhouse 默认跳到 instance
    if (path.endsWith('/database-security/data-asset/clickhouse')) {
      navigate('/database-security/data-asset/clickhouse/instance', { replace: true });
      return;
    }
    if (path.endsWith('/database-security/data-asset/clickhouse/database')) {
  ...
  const handleTabChange = (key: string) => {
    ...
    setActiveTab(next);
    navigate(`/database-security/data-asset/clickhouse/${next}`);
  };

  return (<Layout style={{ padding: '24px' }}><Content><Card title="ClickHouse 数据资产"><Tabs activeKey={activeTab} onChange={handleTabChange}><TabPane tab="实例" key="instance"><Form layout="inline" style={{ marginBottom: 16 }}><Form.Item label="实例"><Input
                          placeholder="请输入实例"
                          allowClear
