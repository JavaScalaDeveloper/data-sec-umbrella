...
const DatabaseSecurity: React.FC = () => {
    ...
    const columns: ColumnsType<Policy> = [
        ...
        },
        {
            ...
            render: (_, record) => (<Space size="middle"><Button icon={<EditOutlined/>}
                                                                 onClick={() => handleEdit(record.id)}>编辑</Button><Button
                icon={<DeleteOutlined/>} danger onClick={() => handleDelete(record.id)}>删除</Button></Space>),
        },
    ];
  ...
  const testRules = async () => {
    ...
    return (<><Layout style={{minHeight: '100vh', width: '100%', margin: 0, padding: 0}}><Sider width={200}
                                                                                                theme="light"><Menu
        mode="inline"
    ...
    /></Sider><Layout style={{width: 'calc(100% - 200px)', margin: 0, padding: 0}}><Content style={{
        padding: '24px',
        marginTop: 0,
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px'}}>
            <Title level={3}>策略管理</Title><Button type="primary" icon={<PlusOutlined/>}>新增策略</Button></div>
        <Tabs activeKey={activeTab} onChange={handleTabChange}><TabPane tab="MySQL" key="mysql"><Card
            style={{marginBottom: '24px'}}><Form
            form={form}
            layout="inline"
        ><Row gutter={16}><Col span={6}><Form.Item name="policy_code" label="策略代码"><Input
            placeholder="请输入策略代码"/></Form.Item></Col><Col span={6}><Form.Item name="policy_name"
                                                                                     label="策略名称"><Input
            placeholder="请输入策略名称"/></Form.Item></Col><Col span={6}><Form.Item name="creator"
                                                                                     label="创建人"><Input
            placeholder="请输入创建人"/></Form.Item></Col><Col span={6}><Form.Item name="sensitivity_level"
                                                                                   label="敏感等级"><Select
            placeholder="请选择敏感等级" style={{width: '100%'}}><Option value={1}>1-低</Option><Option
            value={2}>2-中低</Option><Option value={3}>3-中</Option><Option value={4}>4-中高</Option><Option
            value={5}>5-高</Option></Select></Form.Item></Col><Col span={6}><Form.Item name="hide_example"
                                                                                       label="隐藏样例"><Select
            placeholder="请选择是否隐藏样例" style={{width: '100%'}}><Option value={0}>否</Option><Option
            value={1}>是</Option></Select></Form.Item></Col><Col span={6}><Space><Button type="primary"
                                                                                         icon={<SearchOutlined/>}
                                                                                         onClick={handleSearch}>查询</Button><Button
            onClick={handleReset}>重置</Button></Space></Col></Row></Form></Card><Table
            columns={columns}
            dataSource={policies}
            rowKey="id"
            loading={loading}
            ...
            style={{width: '100%'}}
        /></TabPane><TabPane tab="Clickhouse" key="clickhouse">
            <div style={{textAlign: 'center', padding: '50px'}}>Clickhouse策略管理功能开发中</div>
        </TabPane></Tabs></>) : activeMenu === '1' ? (<Title level={3}>概览</Title>) : activeMenu === '3' ? (
        <Title level={3}>数据源</Title>) : activeMenu === '4' ? (
        <Title level={3}>数据资产</Title>) : activeMenu === '5-1' ? (
        <Title level={3}>实时任务</Title>) : activeMenu === '5-2' ? (
        <Title level={3}>批量任务</Title>) : activeMenu === '6' ? (<Title level={3}>配置中心</Title>) : null}</Content></Layout></Layout><Modal
        title="编辑策略"
    ...
    ><Select placeholder="请选择敏感等级"><Option value={1}>1-低</Option><Option value={2}>2-中低</Option><Option
        value={3}>3-中</Option><Option value={4}>4-中高</Option><Option
        value={5}>5-高</Option></Select></Form.Item><Form.Item
        name="hideExample"
        label="隐藏样例"
        rules={[{required: true, message: '请选择是否隐藏样例'}]}
    ><Select placeholder="请选择是否隐藏样例"><Option value={0}>否</Option><Option
        value={1}>是</Option></Select></Form.Item><Form.Item
        name="databaseType"
        label="数据库类型"
        rules={[{required: true, message: '请选择数据库类型'}]}
    ><Select placeholder="请选择数据库类型"><Option value="MySQL">MySQL</Option><Option
        value="Clickhouse">Clickhouse</Option><Option value="PostgreSQL">PostgreSQL</Option><Option
        value="Oracle">Oracle</Option></Select></Form.Item><Form.Item label="分类规则">
        <div style={{marginBottom: 16}}><Button type="primary" onClick={addRule}
                                                icon={<PlusOutlined/>}>添加规则</Button></div>
        <Table
            columns={[
                {
                    title: '编号',
                ...
                },
                {
                    ...
                    ><Option value="库名">库名</Option><Option value="库描述">库描述</Option><Option
                        value="表名">表名</Option><Option value="表描述">表描述</Option><Option
                        value="列名">列名</Option><Option value="列描述">列描述</Option><Option
                        value="列值">列值</Option></Select>),
                },
                {
                    title: '条件类型',
                    ...
                    ><Option value="包含">包含</Option><Option value="不包含">不包含</Option><Option
                        value="等于">等于</Option><Option value="不等于">不等于</Option><Option
                        value="以...开头">以...开头</Option><Option value="不以...开头">不以...开头</Option><Option
                        value="以...结尾">以...结尾</Option><Option value="不以...结尾">不以...结尾</Option><Option
                        value="在...之中">在...之中</Option><Option value="不在...之中">不在...之中</Option><Option
                        value="大于">大于</Option><Option value="小于">小于</Option><Option
                        value="大于等于">大于等于</Option><Option value="小于等于">小于等于</Option><Option
                        value="正则匹配">正则匹配</Option><Option value="内置算法">内置算法</Option></Select>),
                },
                {
                    title: '表达式',
        ...
        >
          ...
          />
            ...
            label="AI规则"><TextArea placeholder="请输入AI规则" rows={4} value={aiRule}
                                     onChange={(e) => setAiRule(e.target.value)}/>
        </Form.Item><Form.Item label="验证数据">
        ...
        </p>{validationResult.ruleDetails && (<div><p>规则命中详情:</p>
            <ul>{validationResult.ruleDetails.map((detail: any, index: number) => (<li key={index}>{detail.rule}:<Tag
                color={detail.matched ? 'green' : 'red'}>{detail.matched ? '命中' : '未命中'}</Tag>({detail.detail})
            </li>))}</ul>
        </div>)}
